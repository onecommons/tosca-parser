    ...
    def test_interfaces(self):
        ...
        for interface in interfaces:
            if interface.name == 'create':
                ...
                self.assertEqual(1, len(interface.outputs))  # defined on node type interface
                ...
                self.assertEqual(1, len(interface.outputs))  # defined on node type interface
                self.assertEqual(interface.outputs["wp_db_created"], {'type': 'integer', 'mapping': 'db_created'})
                out_prop = interface.get_declared_outputs().get("wp_db_created")
                assert out_prop
                assert out_prop.value is None
                assert out_prop.type == "integer"
                assert out_prop.schema.schema["mapping"] == "db_created"
